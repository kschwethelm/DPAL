...
from datasets import MIMIC, CheXpert, get_slni_dataset
...
def get_dataset(args, ddp=False):
    ...
    if args.dataset == "cifar10":
        ...
        train_dataset = CIFAR10(root=args.data_root, train=True, download=True, transform=transforms)
        ...
        train_dataset = OCTMNIST(root=args.data_root, split="train", download=True, transform=transforms, size=img_size)
        test_dataset = OCTMNIST(root=args.data_root, split="test", download=True, transform=test_transform, size=img_size)
        ...
        train_dataset = BloodMNIST(root=args.data_root, split="train", download=True, transform=transforms, size=img_size)
        test_dataset = BloodMNIST(root=args.data_root, split="test", download=True, transform=test_transform, size=img_size)
        ...
        train_dataset = CheXpert(root=os.path.join(args.data_root, "chexpert"), size=img_size, split="train", transform=transforms, memmap_path=os.path.join(args.data_root, f"chexpert/memmap_{img_size}_train.memmap"))
        test_dataset = CheXpert(root=os.path.join(args.data_root, "chexpert"), size=img_size, split="test", transform=test_transform, memmap_path=os.path.join(args.data_root, f"chexpert/memmap_{test_size}_test.memmap"))
        ...
        train_dataset = MIMIC(root=os.path.join(args.data_root, "mimic/mimic-cxr-jpg/2.1.0"), size=img_size, split="train", transform=transforms)
        test_dataset = MIMIC(root=os.path.join(args.data_root, "mimic/mimic-cxr-jpg/2.1.0"), size=test_size, split="test", transform=test_transform)
    ...
    al_datahandler = AL_datahandler(train_dataset, args.dataset, args.initial_dataset_size, args.labeling_budget, args.query_sizes, multi_label=multi_label, random_sampling=not args.al, ddp=ddp)

    sampler = DistributedSampler(test_dataset) if ddp else None
    val_loader = DataLoader(test_dataset, batch_size=args.max_physical_batch_size, shuffle=False, sampler=sampler)
...
def get_model(args, num_channels, num_classes, img_size, device, ddp=False):
    ...
    if args.model == "resnet9":
        model = ResNet9(in_channels=num_channels, num_classes=num_classes, norm_layer='group')
    elif args.model == "eq_resnet9":
        model = EquivariantResNet9(input_channels=num_channels, num_classes=num_classes, spatial_dims=img_size)
...
def get_optimizer(args, model, ddp=False):
    ...
    if args.optimizer == "sgd":
        ...
        optimizer = SGD_AGC(model.named_parameters(), lr=args.learning_rate, clipping=0.1)
        # Find desired parameters and exclude them from clipping
        for group in optimizer.param_groups:
            name = group['name'] 
            exclude = model.module.exclude_from_clipping(name) if ddp else model.exclude_from_clipping(name)
            if exclude:
                group['clipping'] = None
...
def make_private(model, optimizer, train_loader, al_iterations, args, device, ddp):
    ...
    if rank==0:
        ...
        dataset_sizes = [args.initial_dataset_size+sum(args.query_sizes[:i]) for i in range(al_iterations+1)]
        steps_per_epoch = [(dataset_sizes[i]//args.batch_size)+add_iter
        ...
        if args.epochs_al is None:
            ...
            steps = [steps_per_epoch[i]*args.epochs_al[i] for i in range(al_iterations+1)]
        ...
        ddp_logging(f"Training with eps = {args.epsilon}, delta = {args.delta}, sigma = {noise_multiplier}, clip = {args.max_sample_grad_norm}", rank)
        ...
        if args.dp_al_mode == "naive":
            ...
            steps_new, sample_rates_old, sample_rates_new, noise_multipliers = step_amplification_iterations(
            ...
            sample_rates_old, sample_rates_new, noise_multipliers = noise_reduction_iterations(
                noise_multiplier=noise_multiplier,
                delta=args.delta,
                steps=steps,
                sample_rates=sample_rates,
    ...
    ddp_logging(f"Steps before: {steps}, Steps amplified: {steps_new}, Sample rates old: {sample_rates_old}, Sample rates new: {sample_rates_new}", rank)
    ddp_logging(f"New noise multipliers: {noise_multipliers}", rank)

    return model, optimizer, train_loader, privacy_engine, steps_new, [sample_rates_old, sample_rates_new], noise_multipliers
