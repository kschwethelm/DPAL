...
def create_memmap_file(root, size, split, transform, rgb):
    ...
    paths = annotation["Path"].apply(lambda x: make_new_path(x, root, size, split)).tolist()
    ...
    for i, img_path in tqdm(enumerate(paths)):
        ...
        if i == 0:
            ...
            memmap = np.memmap(f"{root}/memmap_{size}_{split}.memmap", dtype='float32', mode='w+', shape=shape)
...
def main():
    ...
    create_memmap_file(data_root, size, "train", transforms, rgb=True)
    create_memmap_file(data_root, size, "valid", transforms, rgb=True)
    create_memmap_file(data_root, size, "test", transforms, rgb=True)
