...
def select_samples(
    ...
    assert query_size <= len(unlabeled_loader.dataset), "Query size is larger than the number of unlabeled samples."
    ...
    if query_strategy == "random":# or epsilon == 0:
        ...
        if multi_label:
            ...
            max_entropy = 0.8
        indices = entropy_sampling(al_iter, data_name, model, unlabeled_loader, query_size, epsilon, device, multi_label, ddp, max_entropy=max_entropy)
    elif query_strategy == "confidence":
        ddp_logging("Confidence sampling...", rank)
        indices = least_confidence(data_name, model, unlabeled_loader, query_size, epsilon, device, multi_label, ddp)
    elif query_strategy == "margin":
        assert not multi_label, "Margin sampling does not work in multi_label classification."
        ddp_logging("Margin sampling...", rank)
        indices = minimum_margin(data_name, model, unlabeled_loader, query_size, epsilon, device, ddp)
    elif query_strategy == "bald":
        indices = bald_sampling(data_name, model, unlabeled_loader, query_size, epsilon, device, multi_label, ddp, num_models=20, dropout=0.3)
    elif query_strategy == "badge":
        indices = badge_sampling(data_name, model, unlabeled_loader, query_size, epsilon, device, multi_label, ddp)
    else:
...
class AL_datahandler():
    def __init__(self, dataset, data_name, initial_labeled_size, labeling_budget, query_sizes, random_sampling, multi_label, ddp=False):
        assert random_sampling or (sum(query_sizes) + initial_labeled_size == labeling_budget), "Query sizes not correct. Num labeled will be < labeling budget."
    ...
    def get_labeled_loader(self, batch_size):
        labeled_loader = DataLoader(self.labeled_dataset, batch_size=batch_size, shuffle=True, drop_last=True)
        return labeled_loader
    
    def get_unlabeled_loader(self, batch_size):
        unlabeled_loader = DataLoader(self.unlabeled_dataset, batch_size=batch_size, shuffle=False, drop_last=False)
        return unlabeled_loader

    def get_new_labeled_loader(self, batch_size):
        new_labeled_dataset = Subset(self.dataset, self.new_indices)
        new_labeled_loader = DataLoader(new_labeled_dataset, batch_size=batch_size, shuffle=True, drop_last=True)
        return new_labeled_loader
    
    def get_dataloaders(self, batch_size):
        return self.get_labeled_loader(batch_size), self.get_unlabeled_loader(batch_size)
    
    def label_data(self, indices):
        assert len(indices) <= len(self.unlabeled_indices), "Too many samples queried."
        assert len(self.labeled_indices)+len(indices) <= self.labeling_budget, "Labeling budget exceeded."
    ...
    def al_iteration(self, al_iter, model, query_strategy, query_size, batch_size, epsilon, device):
