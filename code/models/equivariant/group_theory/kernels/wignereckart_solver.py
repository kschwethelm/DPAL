...
from ....equivariant.group_theory.representations import IrreducibleRepresentation

from typing import Union, Tuple, Dict, Iterable
from collections import defaultdict

__all__ = ["WignerEckartBasis", "RestrictedWignerEckartBasis"]
    ...
    def __init__(
        ...
        _coeff = [
            torch.einsum(
                ...
                "mnsi,koi->ksmno",
                torch.tensor(
                    group.clebsch_gordan_coeff(self.n, self.m, j), dtype=torch.float32
                ),
...
class RestrictedWignerEckartBasis(IrrepBasis):
        ...
        for _j in _js:
            ...
            coeff = [
                torch.einsum(
                    ...
                    "nmsi,kji,jyt->kstnmy",
                    torch.tensor(
                        G.clebsch_gordan_coeff(self.n, self.m, j), dtype=torch.float32
                    ),
    ...
    def attrs_j_iter(self, j: Tuple) -> Iterable:
        ...
        for _j, _jj in self._js_restriction[j]:
            _jJl = self.group.clebsch_gordan_coeff(self.n, self.m, _j).shape[2]
