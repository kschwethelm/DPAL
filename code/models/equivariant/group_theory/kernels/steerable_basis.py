...
class IrrepBasis(KernelBasis):
    def __init__(
        ...
        r"""
        Abstract class for bases implementing the kernel constraint solutions associated to irreducible
        input and output representations.
        .. note ::
            The steerable *filter* ``basis`` is not necessarily associated with the same group as
            ``in_irrep`` and ``out_irrep``.
            For instance, :class:`~RestrictedWignerEckartBasis` uses a larger group to define ``basis``.
            The attribute ``IrrepBasis.group``, instead, refers to the equivariance group of this
            steerable *kernel* basis and is the same group of ``in_irrep`` and ``out_irrep``.
        ...
        self.in_irrep: IrreducibleRepresentation = self.group.irrep(
            *self.group.get_irrep_id(in_irrep)
        )
        self.out_irrep: IrreducibleRepresentation = self.group.irrep(
            *self.group.get_irrep_id(out_irrep)
        )
    ...
    def sample_harmonics(
        ...
        sampled on this points using the method :meth:`SteerableFilterBasis.sample_as_dict` of
        ``self.basis``.
        ...
        `m` is the multiplicity of the irrep (see :meth:`~SteerableKernelBasis.dim_harmonic`)
...
class SteerableKernelBasis(KernelBasis):
    def __init__(
        ...
        **kwargs,
        ...
        space of all convolutional  It follows that any equivariant kernel can be expressed in terms of a basis
        of this space.

        This class solves the kernel constraint for two arbitrary representations by combining the solutions of the
        kernel constraints associated to their :class:`~IrreducibleRepresentation` s.
        In order to do so, it relies on ``irreps_basis`` which solves individual irreps constraints. ``irreps_basis``
        must be a class (subclass of :class:`~IrrepsBasis`) which builds a basis for equivariant
        ...
        Therefore, equivariance does not enforce any constraint on the radial component of the
        Hence, this class only implements a basis for the angular part of the
            ...
            for o_irrep_id in set(out_repr.irreps):
                try:
                    ...
                    intertwiner_basis = irreps_basis._generator(
                        basis, i_irrep_id, o_irrep_id, **kwargs
                    )
        ...
        self.bases = [
            [None for _ in range(len(out_repr.irreps))]
            for _ in range(len(in_repr.irreps))
        ]
        dim = 0
        # loop over all input irreps
            ...
            for oo, o_irrep_id in enumerate(out_repr.irreps):
                if (i_irrep_id, o_irrep_id) in self.irreps_bases:
                    ...
                    for j in self.bases[ii][oo].js:
                        self._dim_harmonics[j] += self.irreps_bases[
                            (i_irrep_id, o_irrep_id)
                        ].dim_harmonic(j)
    ...
    def compute_harmonics(self, points: torch.Tensor) -> Dict[Tuple, torch.Tensor]:
        ...
        .. seealso ::
            :meth:`SteerableFiltersBasis.sample_as_dict`.
        """
    ...
    def sample_harmonics(
        ...
        Sample the continuous basis elements on the discrete set of points.
        Rather than using the points' coordinates, the method directly takes in input the steerable basis elements
        sampled on this points using the method :meth:`SteerableKernelBasis.compute_harmonics`.
        Similarly, rather than returning a single tensor containing all sampled basis elements, it groups basis elements
        by the ``G``-irrep acting on them.
        The method returns a dictionary mapping each irrep's ``id`` to a tensor of shape `(N, m, o, i)`, where
        `N` is the number of points,
    ...
    def _change_of_basis(
        self, samples: Dict[Tuple, torch.Tensor], out: Dict[Tuple, torch.Tensor] = None
    ) -> Dict[Tuple, torch.Tensor]:
        ...
        for j in samples.keys():
            if self.A_inv is not None and self.B is not None:
                out[j][:] = torch.einsum(
                    "no,pboi,ij->pbnj",
                    self.B.to(samples[j].dtype),
                    samples[j],
                    self.A_inv.to(samples[j].dtype),
                )
            elif self.A_inv is not None:
                out[j][:] = torch.einsum(
                    "pboi,ij->pboj", samples[j], self.A_inv.to(samples[j].dtype)
                )
            elif self.B is not None:
                out[j][:] = torch.einsum(
                    "no,pboi->pbni", self.B.to(samples[j].dtype), samples[j]
                )
            else:
                out[j][...] = samples[j]
    ...
    def __eq__(self, other):
        if not isinstance(other, SteerableKernelBasis):
            ...
            or self.in_repr != other.in_repr
            or self.out_repr != other.out_repr
        ):
            return False
        else:
