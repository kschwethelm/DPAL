...
from ...equivariant import nn
from ...equivariant.group_theory import Group, GroupElement, Representation, directsum
...
class FieldType:
    def __init__(
        ...
        # nn.GSpace: Space where data lives and its (abstract) symmetries
        self.gspace = gspace
        ...
        # in :attr:`nn.fieldtype.representations`, its size is the sum of each of these
        ...
        self._hash = hash(
            self.gspace.name
            + ": {"
            + ", ".join([r.name for r in self.representations])
            + "}"
        )
    ...
    def representation(self) -> Representation:
        ...
        if self._representation is None:
            ...
            self._representation = directsum(
                ...
                name=f"FiberRepresentation:[{self.size}], [{uniques_fields_names}]",
            )
    ...
    def transform_fibers(
        self, input: torch.Tensor, element: GroupElement
    ) -> torch.Tensor:
        r"""
        ...
        """
        representation = self.fiber_representation(element).to(
    ...
    def transform(
        ...
        if coords is None:
            ...
            transformed = self.gspace._interpolate_transform_basespace(
                output_np, element, order=order
            )
            ...
            assert (
                coords.shape[1] == self.gspace.dimensionality
    ...
    def __repr__(self):
        ...
        return (
            ...
            + self.gspace.name
            + ": {"
            + ", ".join([f"{k} (x{n})" for k, n in summarized_representations])
            + "}"
            + f"({self.size})]"
        )
    ...
    def __call__(
        self, tensor: torch.Tensor, coords: torch.Tensor = None
    ) -> "nn.GroupTensor":
        return nn.GroupTensor(tensor, self, coords)