...
class InducedNormNonLinearity(EquivariantModule):
    def __init__(self, in_type: FieldType, function: str = "n_relu", bias: bool = True):
        ...
        # Group fields by size and the size of the subfields and retrieve the indices of the fields
        ...
        for id in list(self._nfields.keys()):
            ...
            self.indices[id] = _indices[id].to(f"cuda:{torch.cuda.current_device()}")

        if bias:
            # build a bias for each field
            self.log_bias = Parameter(
                torch.zeros(len(self.in_type), dtype=torch.float), requires_grad=True
            )
        else:
            self.log_bias = None
    ...
    def forward(self, input: GroupTensor) -> GroupTensor:
        ...
        for id in self._order:
            ...
            norms = fm.view(b, -1, subfield_size, *spatial_dims).norm(
                dim=2, keepdim=True
            )
        ...
        # multiply the input by the multipliers computed and wrap the result in a GroupTensor
