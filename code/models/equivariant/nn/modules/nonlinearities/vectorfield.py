...
from .....equivariant.group_theory import CyclicGroup
...
class VectorFieldNonLinearity(EquivariantModule):
    def __init__(self, in_type: FieldType, **kwargs):
        ...
        for r in in_type.representations:
            assert (
                "vectorfield" in r.supported_nonlinearities
            ), 'Error! Representation "{}" does not support "vector-field" non-linearity'.format(
                r.name
            )

            assert (
                r.name == "regular"
                and r.size == in_type.gspace.fibergroup.rotation_order
            ), r.name
        ...
        self._rotations = self.space.fibergroup.rotation_order
