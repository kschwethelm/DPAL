...
class GatedNonLinearity2(EquivariantModule):
    def __init__(self, in_type: Tuple[FieldType, FieldType], **kwargs):
        r"""
        ...
        for s in list(self._nfields.keys()):
            _input_indices[s] = torch.LongTensor(
                [min(_input_indices[s]), max(_input_indices[s]) + 1]
            )
    ...
    def forward(self, gates: GroupTensor, input: GroupTensor) -> GroupTensor:
        r"""

        Apply the gated non-linearity to the input feature map.

        Args:
            gates (GroupTensor): feature map corresponding to the gates
            input (GroupTensor): input feature map corresponding to the gated fields
        ...
        assert isinstance(gates, GroupTensor)
        ...
        # transform the gates
        gates = torch.sigmoid(gates - self.bias.view(1, -1, *[1] * len(spatial_dims)))
        ...
        for size in self._order:
            ...
            output[:, indices[0] : indices[1], ...] = (
                input[:, indices[0] : indices[1], ...].view(b, -1, size, *spatial_dims)
                * g
    ...
    def evaluate_output_shape(
        self, *input_shapes: Tuple[int, int, int, int]
    ...
    def check_equivariance(
        ...
        x = [GroupTensor(x[i], self.in_type[i]) for i in range(2)]
