...
class PointwiseMaxPool(EquivariantModule):
    ...
    def evaluate_output_shape(
        ...
        ho = (
            hi + 2 * self.padding[0] - self.dilation[0] * (self.kernel_size[0] - 1) - 1
        ) / self.stride[0] + 1
        wo = (
            wi + 2 * self.padding[1] - self.dilation[1] * (self.kernel_size[1] - 1) - 1
        ) / self.stride[1] + 1
...
class PointwiseMaxPoolAntialiased(PointwiseMaxPool):
    def __init__(
        ...
        super(PointwiseMaxPoolAntialiased, self).__init__(
            in_type, kernel_size, stride, padding, dilation, ceil_mode
        )
        ...
        grid_x = (
            torch.arange(filter_size).repeat(filter_size).view(filter_size, filter_size)
        )
        grid_y = grid_x.t()
        grid = torch.stack([grid_x, grid_y], dim=-1)

        mean = (filter_size - 1) / 2.0
        variance = sigma**2.0

        # setting the dtype is needed, otherwise it becomes an integer tensor
        r = -torch.sum((grid - mean) ** 2.0, dim=-1, dtype=torch.get_default_dtype())
        ...
        _filter = _filter.view(1, 1, filter_size, filter_size).repeat(
            (in_type.size, 1, 1, 1)
        )
