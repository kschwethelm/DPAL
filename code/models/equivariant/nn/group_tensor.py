...
from ...equivariant.group_theory import GroupElement
...
__all__ = ["GroupTensor", "tensor_directsum"]
...
class GroupTensor:
    def __init__(self, tensor: Tensor, type: FieldType, coords: Tensor = None):
        ...
        A GroupTensor can be interpreted as a *typed* tensor.
        It is wrapping a common :class:`torch.Tensor` and endows it with a (compatible) :class:`~nn.FieldType` as
        *transformation law*.

        The :class:`~nn.FieldType` describes the action of a group :math:`G` on the tensor.
        ...
        All *torch-ecnn* neural network operations have :class:`~nn.GroupTensor` s as inputs and outputs.
        They perform a dynamic typechecking, ensuring that the transformation laws of the data and the operation match.
        See also :class:`~nn.EquivariantModule`.
        ...
        The base space action will then transform these coordinates (no interpolation required).
        See also the method :meth:`nn.FieldType.transform` for more details.

        The operations of **addition** and **scalar multiplication** are supported.
        For example::

            gs = rot2dOnR2(8)
            type = nn.FieldType(gs, [gs.regular_repr]*3)
            t1 = nn.GroupTensor(torch.randn(1, 24, 3, 3), type)
            t2 = nn.GroupTensor(torch.randn(1, 24, 3, 3), type)
        ...
        .. warning ::
            The multiplication of a PyTorch tensor containing only a scalar with a GroupTensor is only supported
        ...
        A GroupTensor can also be right-multiplied with a :class:`~GroupElement` with the ``@`` operator.
        The group element must belong to the :attr:`~nn.FieldType.fibergroup` of the :class:`~nn.FieldType`
        ``type`` of this tensor.
        ...
        This operation is equivalent to :meth:`~nn.GroupTensor.transform_fibers`.
        Check its documentation for more details.


        A GroupTensor supports **slicing** in a similar way to PyTorch's :class:`torch.Tensor`.
        ...
        first dimension.
        Moreover, a GroupTensor also partially supports **advanced indexing** (see NumPy's
        documentation about
        ...
        .. warning ::
            ...
            In this way, the resulting tensor can always be interpreted as a GroupTensor.


        We give few examples to illustrate this behavior::

            # Example of GroupTensor slicing
            space = rot2dOnR2(4)
            type = nn.FieldType(space, [
            ...
            geom_tensor = nn.GroupTensor(torch.randn(10, type.size, 9, 9), type)
            # or, equivalently:
            ...
            >> torch.Size(1, 2, 9, 9)
            # we can use a boolean tensor to perform advanced indexing over the fields of a group tensor.
        ...
        if coords is None:
            assert len(tensor.shape) == 2 + type.gspace.dimensionality, (
                tensor.shape,
                type.gspace.dimensionality,
            )
        else:
            assert len(coords.shape) == 2, coords.shape

            assert (
                coords.shape[1] == type.gspace.dimensionality
    ...
    def restrict(self, id) -> "GroupTensor":
        r"""
        Restrict the field type of this tensor.

        The method returns a new :class:`~nn.GroupTensor` whose :attr:`~nn.GroupTensor.type`
        is equal to this tensor's :attr:`~nn.GroupTensor.type`
        restricted to a subgroup :math:`H<G` (see :meth:`nn.FieldType.restrict`).
        The restricted :attr:`~nn.GroupTensor.type` is associated with the restricted representation
        :math:`\Res{H}{G}\rho` of the :math:`G`-representation :math:`\rho` associated to this tensor's
        :attr:`~nn.GroupTensor.type`.
        The input ``id`` specifies the subgroup :math:`H < G`.

        Notice that the underlying :attr:`~nn.GroupTensor.tensor` instance will be shared between
        ...
        .. warning ::
            ...
            we suggest to use :class:`nn.RestrictionModule` which pre-computes the new representation offline.

        .. seealso ::

            Check the documentation of the :meth:`~GSpace.restrict` method in the
            :class:`~GSpace` instance used for a description of the parameter ``id``.


        Args:
            id: the id identifying the subgroup :math:`H` the representations are restricted to
        Returns:
            the group tensor with the restricted representations

        """
        new_class = self.type.restrict(id)
        return GroupTensor(self.tensor, new_class, self.coords)
    ...
    def split(self, breaks: List[int]):
        ...
        (see :attr:`nn.FieldType.representations`), the :math:`j`-th output tensor will contain the fields
        ...
        Example ::
            ...
            geom_tensor = nn.GroupTensor(torch.randn(10, type.size, 7, 7), type)
        ...
        Returns:
            list of :class:`~nn.GroupTensor` s into which the original tensor is chunked
        ...
        for b in breaks:
            assert (
                b > last_field
            ), 'Error! "breaks" must be an increasing list of positive indexes'
            ...
            tensors.append(GroupTensor(data, repr, self.coords))

            last_field = b

        return tensors

    def transform(self, element: GroupElement) -> "GroupTensor":
        r"""
        ...
        return GroupTensor(transformed, self.type, coords)

    def transform_fibers(self, element: GroupElement) -> "GroupTensor":
        r"""
        ...
        .. seealso ::
            ...
            See :meth:`nn.GroupTensor.transform` to transform the whole tensor.
        ...
        return GroupTensor(transformed_tensor, self.type, self.coords)
    ...
    def to(self, *args, **kwargs):
        ...
        :class:`~nn.GroupTensor` with the same type.

        .. warning ::
            This method does not affect ``self.coords``.
        """
        tensor = self.tensor.to(*args, **kwargs)
        return GroupTensor(tensor, self.type, self.coords)
    
    def detach(self, *args, **kwargs):
        tensor = self.tensor.detach(*args, **kwargs)
        return GroupTensor(tensor, self.type, self.coords)

    def __getitem__(self, slices) -> "GroupTensor":
        r"""
        ...
        if isinstance(slices, tuple):
            if len(slices) > len(self.tensor.shape):
                raise TypeError(
                    f"""
                        Error! Too many slicing indices for GroupTensor.
        ...
        for i, idx in enumerate(slices):
            if isinstance(idx, slice) or isinstance(idx, int) or idx == Ellipsis:
                ...
                raise TypeError(
                    f"""
                        Error! Advanced Indexing over a GroupTensor is not fully supported yet.
        ...
        if len(expanded_idxs) == 1:
        ...
        elif isinstance(expanded_idxs[1], slice) and (
            expanded_idxs[1].step is None or expanded_idxs[1].step == 1
        ):
            ...
            stop = (
                expanded_idxs[1].stop
                if expanded_idxs[1].stop is not None
                else len(self.type)
            )
            channel_idxs = slice(
                self.type.fields_start[start], self.type.fields_end[stop - 1], 1
            )
            ...
            if start == 0 and stop == len(self.type):
                ...
                type = FieldType(
                    self.type.gspace, self.type.representations[expanded_idxs[1]]
                )
        ...
        return GroupTensor(sliced_tensor, type, coords)

    def has_same_coords(self, other: "GroupTensor") -> bool:
    ...
    def is_compatible(self, other: "GroupTensor") -> bool:
        if self.type != other.type:
            return False
        return self.has_same_coords(other)

    def __add__(self, other: "GroupTensor") -> "GroupTensor":
        r"""
        Add two compatible :class:`~nn.GroupTensor` using pointwise addition.
        The two tensors needs to have the same shape and be associated to the same field type.
        Args:
            other (GroupTensor): the other group tensor
        Returns:
            the sum
        """
        if isinstance(other, GroupTensor):
            assert self.is_compatible(
                other
            ), "The two group tensor must have the same `type` and `coords`"
            return GroupTensor(self.tensor + other.tensor, self.type, self.coords)
        else:
            return NotImplemented

    def __sub__(self, other: "GroupTensor") -> "GroupTensor":
        r"""
        Subtract two compatible :class:`~nn.GroupTensor` using pointwise subtraction.
        The two tensors needs to have the same shape and be associated to the same field type.
        Args:
            other (GroupTensor): the other group tensor
        Returns:
    ...
    def __iadd__(self, other: "GroupTensor") -> "GroupTensor":
        r"""
        Add a compatible :class:`~nn.GroupTensor` to this tensor inplace.
        The two tensors needs to have the same shape and be associated to the same field type.
        Args:
            other (GroupTensor): the other group tensor
        Returns:
    ...
    def __isub__(self, other: "GroupTensor") -> "GroupTensor":
        r"""
        Subtract a compatible :class:`~nn.GroupTensor` to this tensor inplace.
        The two tensors needs to have the same shape and be associated to the same field type.
        Args:
            other (GroupTensor): the other group tensor
        Returns:
    ...
    def __mul__(self, other: Union[float, torch.Tensor]) -> "GroupTensor":
        r"""
        Scalar product of this :class:`~nn.GroupTensor` with a scalar.
        ...
        if isinstance(other, float) or (
            ...
            return GroupTensor(self.tensor * other, self.type, self.coords)
        else:
            return NotImplemented

    __rmul__ = __mul__

    def __imul__(self, other: Union[float, torch.Tensor]) -> "GroupTensor":
        r"""
        Scalar product of this :class:`~nn.GroupTensor` with a scalar.
    ...
    def __rmatmul__(self, other: GroupElement):
        ...
        Equivalent to :meth:`nn.GroupTensor.transform_fibers`.
        ...
        if isinstance(other, GroupElement):
            ...
            return GroupTensor(
                self.type.transform_fibers(self.tensor, other), self.type, self.coords
            )
        else:
            return NotImplemented
...
def tensor_directsum(tensors: List["GroupTensor"]) -> "GroupTensor":
    r"""
    Concatenate a list of :class:`~nn.GroupTensor` s on the channels dimension (``dim=1``).
    ...
    In the resulting :class:`~nn.GroupTensor`, the channels dimension will be associated with the direct sum
    ...
    Args:
        tensors (list): a list of :class:`~nn.GroupTensor` s
    Returns:
    ...
    # build the new group tensor
    return GroupTensor(data, cls, tensors[0].coords)
