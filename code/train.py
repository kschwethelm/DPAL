...
def getArguments():
    ...
    parser = argparse.ArgumentParser(
        description="Active learning classification training with Differential Privacy",
        add_help=True,
    )
    ...
    # Output settings
    parser.add_argument(
        "--exp_name", "-n", default="test", type=str, help="Name of the experiment."
    )
    parser.add_argument(
        "--output_dir",
        default="output",
    ...
    #    Training parameters
    parser.add_argument(
        "--seed", "-s", default=1, type=int, help="Set seed for deterministic training."
    )
    parser.add_argument(
        "--epochs",
        "-e",
    ...
    parser.add_argument(
        "--epsilon", "-eps", default=8, type=float, help="Target epsilon for DP-SGD"
    )
    parser.add_argument(
        "--delta", "-delta", default=1e-5, type=float, help="Privacy delta for DP-SGD"
    )
    parser.add_argument(
        "--max_sample_grad_norm",
        "-clip",
        default=1.0,
        type=float,
    ...
    if args.epochs_al is not None:
        ...
        assert len(args.epochs_al) == len(args.query_sizes) + 1, (
            "Number of epochs_al must match number of query_sizes."
        )
...
def main(rank, world_size, args):
    ...
    al_datahandler, val_loader, num_channels, num_classes, multi_label, img_size = (
        get_dataset(args, ddp)
    )
    ...
    (
        ...
        model, optimizer, train_loader, al_datahandler.al_iterations, args, device, ddp
    )
    ...
    for al_iter in range(al_datahandler.al_iterations + 1):
        if al_iter > 0 and args.al:
            ddp_logging("AL: Querying new labels...", rank)
            ...
            sample_rates_per_instance = [sample_rates[0][al_iter]] * (
                args.initial_dataset_size + sum(args.query_sizes[: (al_iter - 1)])
            ) + [sample_rates[1][al_iter]] * args.query_sizes[al_iter - 1]
            trainer.train_loader = DPDataLoader.from_data_loader(
                train_loader,
                steps=steps[al_iter],
                custom_sample_rate=sample_rates_per_instance,
                distributed=ddp,
            )
            ...
            ddp_logging(
                f"AL iter ({al_iter}/{al_datahandler.al_iterations}): Labeled {args.query_sizes[al_iter - 1]} samples with the highest entropy.",
                rank,
            )
