...
from opacus.utils.uniform_sampler import (
    ...
    PoissonIndividualSampler,
    DistributedPoissonIndividualSampler,
)
...
class DPDataLoader(DataLoader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if distributed and not isinstance(sample_rate, list):
            batch_sampler = DistributedUniformWithReplacementSampler(
                total_size=len(dataset),  # type: ignore[assignment, arg-type]
                sample_rate=sample_rate,
                generator=generator,
            )
        elif distributed and isinstance(sample_rate, list):
            assert len(sample_rate) == len(dataset)
            batch_sampler = DistributedPoissonIndividualSampler(
            ...
            assert len(sample_rate) == len(dataset)
            batch_sampler = PoissonIndividualSampler(
    ...
    def from_data_loader(
        cls, data_loader: DataLoader, *, distributed: bool = False, steps: int=None, custom_sample_rate : Union[float, List[float]] = None, generator=None
    ):
        """
        ...
        return cls(
            ...
            sample_rate=1 / len(data_loader) if custom_sample_rate is None else custom_sample_rate,
            steps=steps,
